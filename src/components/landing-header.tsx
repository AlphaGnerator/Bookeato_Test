...
export function LandingHeader() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Only change state if we actually scrolled a meaningful amount
  ...
  return (
    <header className={cn(
      "fixed top-0 left-0 right-0 z-50 border-none transform transition-transform duration-300 ease-in-out bg-[#3bc166] shadow-md",
          ...
          {getNavLinks().map((link) => (
            link.isDropdown ? (
                 <DropdownMenu key={link.href}>
                     <DropdownMenuTrigger className="text-sm font-medium text-slate-300 transition-colors hover:text-white focus:outline-none flex items-center gap-1">
                         {link.label}
                     </DropdownMenuTrigger>
                     <DropdownMenuContent>
