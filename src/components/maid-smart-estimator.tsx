...
import { Minus, Plus, Check, ArrowLeft, Calendar as CalendarIcon, Clock, CalendarDays } from 'lucide-react';
...
export function MaidSmartEstimator({ onNext, onBack }: { onNext?: () => void, onBack?: () => void }) {
  ...
  useEffect(() => {
    ...
    if (serviceParam) {
      const keys = serviceParam.split(',').map(s => s.toLowerCase().trim());
      setChores(prev => ({
        ...prev,
        houseCleaning: keys.includes('house_cleaning') || keys.includes('housecleaning'),
        sweeping: keys.includes('sweeping') || keys.includes('mopping'),
        utensils: keys.includes('utensils') || keys.includes('kitchen'),
        bathroom: keys.includes('bathroom') || keys.includes('sanitization'),
        laundry: keys.includes('laundry') || keys.includes('ironing'),
        dusting: keys.includes('dusting') || keys.includes('appliance'),
        misc: keys.includes('misc') || keys.includes('others'),
      }));
  ...
  }, [hoursParam, planParam, searchParams]);
  ...
  const houseCleaningTime = chores.houseCleaning ? (homeSize === '1 BHK' ? 30 : homeSize === '2 BHK' ? 45 : homeSize === '3 BHK' ? 60 : 75) : 0;
  ...
  const dustingTime = chores.dusting ? 20 : 0;
  const miscChoreTime = chores.misc ? (planParam === 'monthly' ? 15 : miscTime) : 0;

  const totalRecommendedMinutes = houseCleaningTime + sweepingTime + utensilsTime + bathroomTime + laundryTime + dustingTime + miscChoreTime;
  const recommendedHours = totalRecommendedMinutes / 60;


  const calculateMonthlyPrice = (hours: number) => {
    const houseCleaningBase = chores.houseCleaning ? (homeSize === '1 BHK' ? 1500 : homeSize === '2 BHK' ? 2200 : homeSize === '3 BHK' ? 3000 : 3800) : 0;
    ...
    const dustingBase = chores.dusting ? 600 : 0;
    const miscBase = chores.misc ? 500 : 0;

    const totalBaseCost = houseCleaningBase + sweepingBase + utensilsBase + bathroomBase + laundryBase + dustingBase + miscBase;
    ...
    if (slotReady) {
      const selectedChores = Object.entries(chores)
        ...
        .map(([name, _]) => {
          ...
          switch(name) {
            case 'houseCleaning':
              estimatedTime = houseCleaningTime;
              choreName = "House Cleaning & Deep Tidying";
              break;
            case 'sweeping': 
            ...
            case 'laundry': 
              ...
              break;
            case 'dusting':
              estimatedTime = dustingTime;
              choreName = "Dusting & Appliance Wipe";
              break;
            case 'misc': 
      ...
      localStorage.setItem('bookeato_maid_estimate', JSON.stringify({
        ...
        selectedTimeSlot: selectedTimeSlot || '09:00 AM',
      }));
      if (onNext) onNext();
    }
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white min-h-[500px] flex flex-col shadow-[0_0_40px_-15px_rgba(0,0,0,0.1)] rounded-[2.5rem] overflow-hidden border border-stone-100 relative pb-28 md:pb-24">
      ...
      <div className="p-6 sm:p-8 space-y-10 flex-1 pt-20 sm:pt-24">
          ...
          <div className="space-y-3">
            ...
            <div 
              className={cn(
                ...
                chores.houseCleaning ? "border-green-500 bg-green-50/30" : "border-stone-100 hover:border-stone-200 bg-white"
              )}
              onClick={() => toggleChore('houseCleaning')}
            >
              <div className="flex items-center gap-3">
                <div className={cn("w-6 h-6 rounded-md flex items-center justify-center border-2 transition-colors", chores.houseCleaning ? "bg-green-500 border-green-500 text-white" : "border-stone-300")}>
                  {chores.houseCleaning && <Check className="w-4 h-4" strokeWidth={3} />}
                </div>
                <div>
                  <span className="font-bold text-stone-800 block">House Cleaning & Tidying</span>
                  <span className="text-xs text-stone-400">Living room, bedroom dusting & deep cleaning</span>
                </div>
              </div>
              <span className="text-sm font-bold text-stone-500 bg-stone-100 px-3 py-1 rounded-full">{houseCleaningTime} mins</span>
            </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-bold text-stone-800 block">Sweeping & Mopping</span>
                  <span className="text-xs text-stone-400">Complete floor sweeping & wet mopping</span>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-bold text-stone-800 block">Utensils & Kitchen Reset</span>
                  <span className="text-xs text-stone-400">Sink cleaning, dishwashing & kitchen counters</span>
                </div>
              </div>
            ...
            <div className={cn(
              ...
              <div className="flex items-center gap-3 flex-1 cursor-pointer touch-manipulation" onClick={() => toggleChore('bathroom')}>
                ...
                <div>
                  <span className="font-bold text-stone-800 block">Bathroom Sanitization</span>
                  <span className="text-xs text-stone-400">Deep tile scrubbing & toilet sanitizing</span>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-bold text-stone-800 block">Laundry & Ironing</span>
                  <span className="text-xs text-stone-400">Washing, folding & neat ironing</span>
                </div>
              </div>
              <span className="text-sm font-bold text-stone-500 bg-stone-100 px-3 py-1 rounded-full">{laundryTime} mins</span>
            </div>

            {/* Dusting & Appliance Wipe */}
            <div 
              className={cn(
                "flex items-center justify-between p-4 rounded-2xl border-2 transition-colors cursor-pointer touch-manipulation", 
                chores.dusting ? "border-green-500 bg-green-50/30" : "border-stone-100 hover:border-stone-200 bg-white"
              )}
              onClick={() => toggleChore('dusting')}
            >
              <div className="flex items-center gap-3">
                <div className={cn("w-6 h-6 rounded-md flex items-center justify-center border-2 transition-colors", chores.dusting ? "bg-green-500 border-green-500 text-white" : "border-stone-300")}>
                  {chores.dusting && <Check className="w-4 h-4" strokeWidth={3} />}
                </div>
                <div>
                  <span className="font-bold text-stone-800 block">Dusting & Appliance Wipe</span>
                  <span className="text-xs text-stone-400">Furniture, electronics, mirrors & fridge exterior</span>
                </div>
              </div>
              <span className="text-sm font-bold text-stone-500 bg-stone-100 px-3 py-1 rounded-full">{dustingTime} mins</span>
            </div>
            ...
            <div className={cn(
              ...
              <div className="flex items-center gap-3 flex-1 cursor-pointer touch-manipulation" onClick={() => toggleChore('misc')}>
                ...
                <div>
                  <span className="font-bold text-stone-800 block">Miscellaneous Chores</span>
                  <span className="text-xs text-stone-400">Balcony, garbage disposal & general help</span>
                </div>
              </div>
        ...
        {/* 3. Date & Time Slot Selection (Below Chores) */}
        <div className="space-y-4 pt-2">
          <h3 className="text-xl font-bold text-stone-900 flex items-center gap-2">
            <CalendarDays className="w-5 h-5 text-green-600" />
            Select Visit Date & Time Slot
          </h3>
          <p className="text-stone-500 font-medium text-xs">
            Choose when you want your professional maid to arrive.
          </p>

          {/* Date Selection Pills + Popover */}
          <div className="flex flex-wrap items-center gap-2 pt-1">
            {quickDates.map((d, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "px-4 py-2.5 rounded-full text-xs font-bold transition-all active:scale-95 flex items-center gap-1.5",
                    isSelected 
                      ? "bg-green-600 text-white shadow-md shadow-green-600/20 scale-[1.02]" 
                      : "bg-stone-50 text-stone-600 hover:bg-stone-100 border border-stone-200"
                  )}
                >
                  <span>{idx === 0 ? 'Tomorrow' : format(d, 'EEE, MMM d')}</span>
                </button>
              );
            })}
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button type="button" className="px-3.5 py-2.5 rounded-full text-xs font-bold bg-stone-50 text-stone-600 hover:bg-stone-100 border border-stone-200 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 pt-2">
            {timeSlots.map((slot) => (
              <button
                ...
                className={cn(
                  "py-3 px-3 rounded-xl text-xs font-bold transition-all border text-center flex items-center justify-center gap-1.5 touch-manipulation",
