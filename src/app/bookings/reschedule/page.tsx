...
import { format, parseISO, addHours, startOfToday, isAfter, differenceInHours } from 'date-fns';
...
export default function ReschedulePage() {
    ...
    if (!booking) {
        return (
            <AppLayout pageTitle="Reschedule">
    ...
    const handleReschedule = async () => {
        if (!user || !bookingId || !newDate || !newTime) return;
        ...
        setIsUpdating(true);
        try {
            const bRef = doc(bookingRef.firestore, `customers/${user.uid}/bookings/${bookingId}`);
            await updateDoc(bRef, {
                bookingDate: selectedDateTime.toISOString(),
                rescheduledAt: serverTimestamp(),
                // Keep the same status 'confirmed' or reset to 'pending' if partner needs to re-approve
                // For now, assume it stays confirmed but date changes
            });
