...
export function BookingSummary({ date }: { date: string }) {
  const { user, draftBookings, dishes, isInitialized, guestConfig, setGuestConfig, addOrUpdateDraftBooking, removeDraftBooking, checkBalance, executeUnifiedCheckout, updateUserProfile } = useCulinaryStore();
  ...
  useEffect(() => {
    if (pincode && !pincodeInput) {
      setPincodeInput(pincode);
    }
  }, [pincode]);

  const activePincode = pincodeInput || pincode;
  const initialLoc = useMemo(() => getLocationFromPincode(activePincode), [activePincode]);

  const [city, setCity] = useState(guestConfig?.city || initialLoc.city || '');
  const [state, setState] = useState(guestConfig?.state || initialLoc.state || '');

  // Keep state & city synced automatically whenever pincode changes or updates
  useEffect(() => {
    if (activePincode) {
      const location = getLocationFromPincode(activePincode);
      if (location.state) {
  ...
  const handlePincodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPincodeInput(val);
    const location = getLocationFromPincode(val);
    if (location.state) {
      setState(location.state);
    }
    if (location.city) {
    ...
    if (firebaseUser) {
      updateUserProfile({ pincode: val });
    } else {
      setGuestConfig({
        pincode: val,
        familySize: familySizeForCalc,
  ...
  const currentAddress = firebaseUser ? (user.address || '') : (guestConfig?.address || '');
  ...
  const handleAddressChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      if (firebaseUser) {
          ...
          setGuestConfig({
              pincode: pincode || guestConfig?.pincode || '',
              familySize: familySizeForCalc,
              city: city || guestConfig?.city || '',
              state: state || guestConfig?.state || '',
              ...guestConfig,
              address: val,
          });
      }
                        ...
                        <CardContent className="space-y-4 px-4 pb-4">
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div className="space-y-1.5">
                                    <Label className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider flex items-center justify-between">
                                        Area Pincode <Badge variant="outline" className="text-[9px] h-4 px-1">Verified</Badge>
                                    </Label>
                                    <Input 
                                        value={pincodeInput}
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="font-bold text-base md:text-lg pt-2">{familySizeForCalc} {familySizeForCalc > 1 ? 'people' : 'person'}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm pt-2">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Input 
                                        value={state || (activePincode ? getLocationFromPincode(activePincode).state : '')}
                                        readOnly
                                        disabled
                                ...
                                <Label className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider flex items-center justify-between">
                                    ...
                                    {firebaseUser && (
                                        <span className="text-[10px] font-semibold text-primary flex items-center gap-1">
                                            <BadgeCheck className="h-3 w-3 text-primary" /> Auto-saved to profile
                                        </span>
                                    )}
                                </Label>
                                <div className="relative group">
                                    <Textarea 
                                        ...
                                        className={cn(
                                            ...
                                            isAddressMissing 
                                                ? "border-amber-400/60 shadow-[0_0_15px_rgba(245,158,11,0.15)] ring-1 ring-amber-400/20" 
                                    ...
                                    />
                                    {isAddressMissing && (
                                        <p className="text-[10px] font-bold text-amber-600 dark:text-amber-500 mt-1.5 animate-pulse uppercase tracking-tight flex items-center gap-1">
                                            <AlertTriangle className="h-3 w-3" /> Please fill your full address (at least 5 characters) to continue
                                        </p>
                                    )}
                                </div>
                            </div>
