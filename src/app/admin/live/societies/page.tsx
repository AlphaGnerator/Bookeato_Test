...
function LiveSocietiesManager() {
  ...
  const seedHyderabadSocieties = async () => {
    if (!firestore) return;
    try {
      const defaultHyderabad = [
        { name: "My Home Vihanga", locationDetails: "Nanakramguda, Financial District, Hyderabad", isActive: true },
        { name: "Prestige High Fields", locationDetails: "ISB Road, Nanakramguda, Hyderabad", isActive: true },
        { name: "Sumadhura Acropolis", locationDetails: "Nanakramguda, Financial District, Hyderabad", isActive: true },
        { name: "Jayabheri Silicon County", locationDetails: "Nanakramguda, Hyderabad", isActive: true },
        { name: "Rajapushpa Atria", locationDetails: "Golden Mile Road, Nanakramguda, Hyderabad", isActive: true },
        { name: "Incor One City", locationDetails: "Nanakramguda Financial District, Hyderabad", isActive: true }
      ];

      const existingNames = new Set(societies.map(s => s.name.toLowerCase()));
      for (const soc of defaultHyderabad) {
        if (!existingNames.has(soc.name.toLowerCase())) {
          await addDoc(collection(firestore, 'liveSocieties'), soc);
        }
      }
      toast({ title: 'Societies Added', description: 'Pop-ups for Nanakramguda, Hyderabad added!' });
      fetchSocieties();
    } catch (e) {
      toast({ title: 'Error', description: 'Failed to add Hyderabad societies', variant: 'destructive' });
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-stone-900 text-white rounded-[2rem] p-8 -mt-2 gap-4">
        <div>
           ...
           <p className="text-stone-400 mt-2 font-medium">Manage active pop-up locations across Nanakramguda, Hyderabad & beyond.</p>
        </div>
        <div className="flex gap-3">
          <Button onClick={seedHyderabadSocieties} variant="secondary" className="rounded-xl font-bold bg-amber-500 hover:bg-amber-600 text-stone-950">
            + Seed Nanakramguda Societies
          </Button>
